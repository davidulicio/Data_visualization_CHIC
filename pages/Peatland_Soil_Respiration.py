...
register_page(__name__, name="OP Soil Respiration", title='OP SR', description='Peatland Soil Respiration Visualization')

# %% Data reading
# get relative data folder
PATH = pathlib.Path(__file__).parent
DATA_PATH = PATH.joinpath("../datasets").resolve()
FILENAME = 'OP220819.csv'
...
flow_8250 = var_reading(df, units, 'FLOW mean')
flow_870 = var_reading(df, units, 'FLOW mean.1')
voltage_8250 = var_reading(df, units, 'VIN mean')
voltage_870 = var_reading(df, units, 'VIN mean.1')
...
layout = html.Div([
    html.H1('Soil Respiration Data from Peatland Omora, CHIC',
...
def name_to_figure(fig_name):
    if fig_name == 'Diagnostics':
        ...
        fig.add_trace(go.Scatter(x=date, y=voltage_8250[0], mode='lines', name='Voltage LI-8250',
